...
export const Article = () => {
	return (
		<article className={styles.article}> {/* Оборачиваем контент статьи в тег article и применяем стили */}
			<Text as='h1' size={45} weight={800} uppercase dynamicLite> {/* Рендерим заголовок первого уровня, используя компонент Text */}
				Портрет Западной Швейцарии
			</Text>
			<div className={styles.titleDescription}> {/* Оборачиваем описание заголовка в div и применяем стили */}
				<Text size={22} weight={800} uppercase align='center' dynamicLite> {/* Рендерим описание заголовка, используя компонент Text */}
					Примитивист Фиштр расписывает новый бюджетный авиалайнер
				</Text>
			</div>
			<img className={styles.image} src={plane} alt='Картинка самолета' /> {/* Рендерим изображение самолета и применяем стили */}
			<Text dynamic size={18} fontStyle='italic'> {/* Рендерим подпись к изображению, используя компонент Text */}
				Фото: Hans-Peter Gauster , &quot;Bombardier CSeries CS300 HB-JCA&quot; ©
				2017 CC BY-SA 2.0
			</Text>
			<Text dynamic size={18}> {/* Рендерим абзац текста, используя компонент Text */}
